...
//==========================DATA TYPES=================================
enum Action {SHOT, PASS, GOALIEKICK, ENDURANCE, ATTACK, BLOCK, CATCH};
...
struct Player{
  ...
  RiskType risk;
  PlayerType type;
  bool hasBall;//do i have the ball?
  ...
  Player(const TeamNo newTeamNo){
    hasBall = false;//nobody has control of ball
    name = "INVALID";
    ...
    //team1.roster[CN].pas;//call to stat of player in position on team
    sht, pas, end, atk, blk, ca = 0;
    //set goalie coordinates
    if(position == GL){
      if(teamNo == HOME){this->xpos = -10000; this->ypos=0;}//HOME team goal
      else{this->xpos = 10000; this->ypos = 0;}//AWAY team goal
    }//end if this player is a goalie    
  ...
  }//end constructor if reading from file or elsewhere

  //ball handlers o.O
  void dropBall(){hasBall = false;}
  void grabBall(){hasBall = true;}
...
public:
  Player roster[];//roster of players on this team
  ...
  Team(const TeamNo newTeamNo){
    ...
    roster[6];// can use the Position enum type to search through list as well
    for(int teammate = 0 : &roster){//fill each roster with default player
      roster[teammate] = Player(teamNo);
    }//end for
  }//end default team constructor

  void setGoal(const int x, const int y)
    {goal.xpos = x; goal.ypos = y;}
  ...
  //check to see if a player on this team has the ball
  bool hasBall(){
    for(int teammate=0; teammate<6; ++teammate){ 
      if(roster[teammate].hasBall) return true;//this player has the ball
    }
    return false;
  }//end hasBall()
  ...
  void setColors(const string color, const string font){
    for(int player = 0; player<6; ++player){
      roster[player].name = color + name + font;}
  }//end setColors
...
Player &getCarrier(){
  if(team1.hasBall()){//does team1 have ball
    for(int teammate = 0; teammate<=6; ++teammate){
      if(team1.roster[teammate].hasBall) return team1.roster[teammate];
    }//end for
  }//end if team1 has ball
  else if(team2.hasBall()){//thus team2 must have ball
    for(int teammate = 0; teammate<=6; ++teammate){
      if(team2.roster[teammate].hasBall) return team2.roster[teammate];
    }//end for teammates on team2
  }//end else team2 has ball
  else{cout<<"ERROR:  No one has ball!"<<endl;}//WTF GDI FML
}//end getCarrier()
...
vector <Player*> fill_blockers(){
  ...
  for(int teammate = 0 : &teams[DFS]->roster){//go through defending team
    Player *defender = &teams[DFS]->roster[teammate];
    if(get_dist(carX,carY, defender->xpos, defender->ypos) <= engage_distance 
      && defender->position!=GL){//except goalie
...
int rstat(const Player &carrier, const Action action){
  //Basic variables
  int stat = 0;
  
  //Modify boundaries based on risk
  RiskType riskmod = carrier.risk;
  ...
  //Grab stat of player
  PlayerType carTB = carrier.type;//carrier's Type Bonus

  //switch variables
  bool type_bonus = false;//if type is activated
  switch(action){
    case SHOT:{
      stat = carrier.sht+shtbonus;
...
void give_stats(Player &player){
  ...
  while (!playerdata.eof()){
    ...
    else{recap<<"Warning!  Double check player names in files!"<<endl;}
  }
  //level multiplier
  int lvl = atoi(temp.substr(0,pos).c_str());
  ...
  else if(temp.substr(0,pos)=="BH")player.type = BH;
  ...
  else if(temp.substr(0,pos)=="C")player.type = C;
  else if(temp.substr(0,pos)=="NO")player.type = NO;
  else cout<<"ERROR setting player.type in player creation."<<endl;//print test
...
void team_pick(Team &team){
 ...
 //Individual Players
 bool validPlayer = true;//flag for invalid team init
 bool validTeam = true;//team is valid
 while(validTeam){
  validTeam = false;//assumed wrong until tested
  system("cls");
  ++pos;
  if (team.roster[CN].position != BN && team.roster[RF].position != BN 
    && team.roster[LF].position != BN && team.roster[RD].position != BN
    && team.roster[LD].position != BN && team.roster[GL].position != BN)
...
void turnover(){
  for(int teammate = 0; teammate<6; ++teammate){
    team1.roster[teammate].dropBall();//drop ball
    team2.roster[teammate].dropBall();//drop ball
  }//end for team1
}//end turnover() default

//takes ball away from everyone but and gives ball to rebounder
void turnover(Player* rebounder){
  turnover();//take ball from everyone
  for(int teammate = 0; teammate<6; ++teammate){
    if(team1.roster[teammate].name == rebounder->name)
      {team1.roster[teammate].grabBall(); break;}
    else if(team2.roster[teammate].name == rebounder->name)
      {team2.roster[teammate].grabBall(); break;}
    else{cout<<"ERROR: void turnover(Player* rebounder)"<<endl; exit(0);}
  }//end for
  recap<<rebounder->name<<" claims the ball."<<endl<<endl;
...
void loose_ball(){
  ...
  double shortest = 3*radius; //shortest what? why set to this number?
  ...
  for(int teammate=0; teammate<6; ++teammate){//iterate through BOTH 
    Position p = team1.roster[teammate].position;//position of this teammate
    if(p != GL and p != BN){//check to see if not goalie or bench
      distance = get_dist(team1.roster[teammate], ball);//check team 1
      if(distance<=shortest){shortest=distance;}//new distance found
      distance = get_dist(team2.roster[teammate], ball);//check team 2
      if(distance<=shortest){shortest=distance;}//new shortest distance found 
    }//end finding shortest distance to ball  
  }//now compare that distance with others'
  ...
  for(int teammate=0; teammate<6; ++teammate){//look through players of team1 AND team2 to find like distance
    distance = get_dist(team1.roster[teammate], ball);
    if(distance==shortest){closestPlayers.push_back(team1.roster[teammate]);}   
    distance = get_dist(team2.roster[teammate], ball);
    if(distance==shortest){closestPlayers.push_back(team2.roster[teammate]);}//end if
  }//end for
...
void carry_ball(){
  if(team1.hasBall()){optimal_movement(getCarrier(),
    team2.goal.xpos, team2.goal.ypos);}
  else if(team2.hasBall()){optimal_movement(getCarrier(), 
    team1.goal.xpos, team1.goal.ypos);}//end elese if()
  else{cout<<"ERROR carry_ball()!!!!"; exit(0);}//EROR!!!
  ball_refresh();
}//end carry_ball()
...
void offense(Team &team){
  for(int teammate = 0; teammate <5; ++teammate){//exclude goalie
    if(!team.roster[teammate].hasBall){//if this player does NOT have the ball..
...
//===========================================================================
//===========================================================================
//combined form of test pass and test shot.  needs overloading for ball/gl
bool testPass(const Player &carrier, const Player &target, const Action action){
...
vector <Player*> gatherUpfielders(const Player &carrier){
  ...
  if(team1.hasBall()){teams[OFS]=&team1; teams[DFS]=&team2;}
  else{teams[DFS]=&team2; teams[OFS]=&team1;}
  for(int teammate = 0; teammate<5; ++teammate){//5 bc goalie never upfield
    if(carrier.name != teams[OFS]->roster[teammate].name){//if not self
      if(get_dist(carrier, teams[DFS]->goal) > //if carrier distance farther than
        ...
        if(testPass(carrier, teams[OFS]->roster[teammate], PASS)){//if w/n pass range
        upfielders.push_back(&teams[OFS]->roster[teammate]);}//add to list
...
Player & pass_target(const Player &carrier){
  ...
  if(upfielders.size() > 1){//if many eligible bachelors...
    return *upfielders[rand()%upfielders.size()];//hell, who cares?
    //future AI will check for "openness"
  }//end if poole of players
  else if(upfielders.size() == 1){//If only one target...
    if(upfielders[0]==nullptr){//is the list empty??
      Team *team;//decide who is on OFS-offense or DFS-defense
      team1.hasBall()? team=&team1 : team =&team2;
      return team->roster[rand()%6];//random person on team
    }//end no players w/n throw distance
    else{return *upfielders[0];}//return the only player
  }//end if only 1 Player in upfielders list
}//end pass_target()
...
void pass(Player &carrier, const vector <Player*> blockers){
  Player &target = pass_target(carrier); //to whom am i passign
  ...
  for(int blocker = 0; blocker<blockers.size(); ++blocker){//for each blocker
    ...
    if(pass>0){//gets a hand on it
      ...
      recap<<blockers[blocker]->name<<" gets a hand on it!"<<endl;
      ++shtbonus;
  ...
  int traveling = get_dist(carrier, target) / engage_distance;
  while(traveling>0){pass*=.9*pass; --traveling; --timeleft;}//degrade by 10% per "time"
  if(pass>0){//pass successfull!!
    recap<<target.name<<" catches the pass!"<<endl<<endl;
    turnover(&target); //give ball
    shtbonus+=3; return;}//increase sht bonus action was sucessful
  else{loose_ball(); return;}//pass failed.  ball is loose
}//end pass()
...
bool test_shot (const Player &carrier, vector <Player*> blockers){
  ...
  while(traveling>0){shot*=.9*shot; --traveling;}
...
//Checks if the shooter has a feasible chance at making a goal
void shoot(Player carrier, vector <Player*> blockers){
 //get carrier shot stat
 int shot = rstat(carrier, SHOT);
 if(carrier.bonusStat == SHOT){shot += rstat(getGoalie(carrier), SHOT);}
 for(int blocker = 0; blocker<blockers.size(); ++blocker){//for each blocker
 ...
 //Calculate distance ball must travel to reach the goalie
  ...
  while(traveling>0){
    shot*=.9*shot; // degrade through water
    recap<<"..."<<shot;//output
  ...
  if(shot>0){//GGGGGGOOOOOOOAAALAAALALLALALALLALALALAL
    ...
    assign_positions();
    turnover(&teams[goalie->teamNo]->roster[CN]);
 ...
 //Goalie snags it
  ...
  recap << goalie->name << " catches the ball." << endl
        << goalie->name << " throws a pass." << endl;
  vector <Player*> emptyvec;//empty vector
  pass(*goalie, emptyvec);//throw it to anybody you can!
  ...
  if(random == 0 && !disoriented){//dodge check
   ...
   disoriented = true;
    ...
    if(endurance <= 0){//ball is stripped by this tackler
      recap << blockers[blocker] << " hits hard and grabs the ball from " 
          << carrier.name << "!" << endl << endl;
      turnover(blockers[blocker]);
      shtbonus = 0;
      return false;
...
void engage(){
 ...
 if(estimateBlock.size() > 0){//Shoot
  if(test_shot(carrier, estimateBlock)){ 
   recap << carrier.name;
   if(carrier.bonusStat==SHOT){recap << "*";}
   recap << " shoots!" << endl;
   shoot(carrier, estimateBlock); 
 ...
 //No Defenders Left: Pass, Shoot, or Dribble
  ...
  if(test_shot(carrier, estimateBlock)){ 
    ...
    recap << " shoots!" << endl;
    shoot(carrier, estimateBlock); 
  }//end else shoot choice
  else if (upfielders.size()>0){//if there are teammates upfield, pass
    recap << carrier.name;
    if(carrier.bonusStat==PASS){recap << "*";}
    recap << " throws a pass!" << endl;
    pass(carrier, estimateBlock); 
  }//end else if pass choice
  else {//dribble
   if(estimateBlock.size() == 0){ recap << endl; }
   return;    
  }//end else dribble
 }//end if no defenders
}//end engage()
...
int main(){
 ...
 string filename = "";
 filename = teams[HOME]->getName() + " vs " + teams[AWAY]->getName() + ".html";
...
//===================================================================     
 ...
 //Coin Toss. Decide who starts with possession.
  ...
  while (timeleft > 0){
    carry_ball();
    recap << carrier.name << "'s ball at (" << ball.xpos/1000 << ","
          << ball.ypos/1000 << "). Shot Bonus: " << shtbonus << endl;
//print test======these two numbers should be identical
    recap << "Carrier at: (" << carrier.xpos/1000 << ","
          << carrier.ypos/1000 << ")." << endl;
    skirmish(); engage(); ball_refresh(); randomize_bonuses(*teams);
    timeleft -= 2;
...
//===================================================================
 ...
 team1starts2nd? turnover(&teams[HOME]->roster[CN]) :
   turnover(&teams[AWAY]->roster[CN]);
   carrier = getCarrier();//refresh carrier
 recap << carrier.name << " catches the ball." << endl << endl;    
