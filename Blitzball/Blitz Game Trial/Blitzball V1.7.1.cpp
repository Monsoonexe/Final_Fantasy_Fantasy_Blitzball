...
//========================KNOWN ISSUES=======================================
/*
POINTERS OR PASS BY REFERENCE WRITTEN INCORRECTLY.  CHANGING THINGS THAT SHOULDN'T BE CHANGED.... CONSIDER GETTERS AND SETTERS FOR EACH
MEMBER
known issues....
players have been known to switch sides or even duplicate.
particularly the centers.....

Weird ypos for LF and RF in SWeep L and Sweep R 0000
NO GLOBAL VARIABLES!!!!!
lots of possibilities for stale data, ie ball vs goalie vs carrier positions
"ball" should always be held by carrier.  so carrier or goalie or form should
be only destination
*/
//============================PROTOTYPES================================
struct Player;
...
//==============================Fill Blockers===================================
//==============================================================================
void teamNameCheck(const string prompt);//prototype
//Ball is Loose
vector <Player*> fill_blockers(){
  ...
  for(int teammate = 0; teammate<5; ++teammate){//go through defending team
    ...
    if(get_dist(carX,carY, defender->xpos, defender->ypos) <= engage_distance 
      && defender->name!=carrier.name){//except goalie
...
void shoot(vector <Player*> blockers){
 ...
 //Goalie snags it
  ...
  pass(emptyvec);//throw it to anybody you can!
...
void engage(){
 ...
 for(int blocker = 0; blocker < blockers.size(); ++blocker){
  ...
  teamNameCheck(" before battling... ");
  //Physical Vector: END vs ATK
  if(!battle(carrier, estimateTackle)){recap << endl;return;}
  teamNameCheck(" after battling()... ");
  carrier=getCarrier();//refresh just in case
  teamNameCheck(" after getCarrier()... ");//
  ...
  if(estimateBlock.size() > 0){//Shoot
    cout<<"deciding action....."<<endl;
    if(test_shot(carrier, estimateBlock)){
      cout<<"shooting..."<<endl;
      shoot(estimateBlock);
      teamNameCheck(" after shooting()... ");
    }
    else{pass(estimateBlock);
      cout<<"passing...."<<endl;
    teamNameCheck(" passing pass()... ");}
    return;
  }//end if 
 
  else{//No Defenders Left: Pass, Shoot, or Dribble
    cout<<"No defenders... deciding."<<endl;
    teamNameCheck(" before gatherUpfielders.. ");
    vector <Player*> upfielders = gatherUpfielders(carrier);
    teamNameCheck(" after gatherUpfielders ");
    ...
    teamNameCheck(" leaving no defenders... ");
    return;    
  }//end else dribble
}//end engage()
...
void playBall(Team *teams[]){
  teamNameCheck(" entering playBall()... ");
  Player &carrier = getCarrier();
  recap << "The ball is released!" << endl
       << carrier.name << " catches the ball." << endl << endl;
  while(timeleft > 0){
    teamNameCheck(" beginning new 'time' instance. ");
    carrier = getCarrier();teamNameCheck("After getCarrier().");
    carry_ball(); teamNameCheck(" after carry_ball()... ");
    recap << carrier.name << "'s ball at (" << ball.xpos/1000 << ","
          << ball.ypos/1000 << "). Shot Bonus: " << shtbonus << endl;
    teamNameCheck(" entering skirmish()... ");
    skirmish(); teamNameCheck(" between skirmish() and engage... "); 
    engage(); teamNameCheck(" between engage() and ball_refresh()... ");
    ball_refresh(); teamNameCheck(" between ball_r and rando())... ");
    randomize_bonuses(*teams);
    timeleft -= 2;
  }//wash, rince, goooooaaaaallllllll
  teamNameCheck(" leaving playBall()... ");
}//end playBall()
...
//===================================================================   
 ...
 teamNameCheck("Right off the bat....");
