...
//============================PROTOTYPES================================
class Player;
void pass(const vector <Player*> blockers);//recursive function
...
Player &getCarrier();//prototype

//============================BALL=====================================
struct Ball{
  int xpos, ypos;
  Player *carrier;
  Ball(){xpos = 0; ypos = 0; carrier = nullptr;}//default constructor
  Ball(int x, int y){xpos = x; xpos = y; carrier = nullptr;}//goal constructor
}ball;
...
public:
  ...
  bool hasBall(){
    for(int teammate = 0; teammate<6; ++teammate){
      if(getCarrier().name==roster[teammate].name &&
        getCarrier().teamNo==teamNo){return true;}//we have the ball
...
Player &getCarrier(){
  ...
  for(int tm=0; tm<2; ++tm){
    for(int teammate = 0; teammate<6; ++teammate){
      playerNames.push_back(teams[tm]->roster[teammate].name);
    }
  }
  string test = " ";
  for(int player = 0; player<playerNames.size()-1; ++player){
    for(int p=player+1; p<playerNames.size(); ++p){
      if(playerNames[player]==playerNames[p]){
        cout<<"ERROR:  player handling.  multiple people with same name"<<endl;
...
void turnover(Player* rebounder){
  ball.carrier=rebounder;//rebound!
...
void skirmish(){
  ...
  else cout<<"WTF am I doing with my life????"<<endl; getch();
...
vector <Player*> gatherUpfielders(const Player &carrier){
  ...
  if(team1.hasBall()){teams[OFS]=&team1; teams[DFS]=&team2;}
  else{teams[DFS]=&team1; teams[OFS]=&team2;}
...
void engage(){
 ...
 for(int blocker = 0; blocker < blockers.size(); ++blocker){
  ...
  if(estimateBlock.size() > 0){//Shoot
    if(test_shot(carrier, estimateBlock)){shoot(estimateBlock);}
...
//==========================FIRST HALF=============================== 
 ...
 //Coin Toss. Decide who starts with possession. and goes second 
  ...
  teams[HOME]->hasBall()==true? team1starts2nd = false : team1starts2nd = true;
    outputTeamLineup(*teams[HOME]);//print test
    outputTeamLineup(*teams[AWAY]);//print test
  playBall(teams); //playBALLLLLLLLL
    outputTeamLineup(*teams[HOME]);//print test
    outputTeamLineup(*teams[AWAY]);//print test
