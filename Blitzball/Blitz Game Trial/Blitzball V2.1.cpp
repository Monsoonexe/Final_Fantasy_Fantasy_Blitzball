#include <iostream> //lines writing errors to the console and such
...
void loose_ball(const Player &fumbler){//butterfingers!!
  ...
  turnover(rebounder); recap<<endl; recapCarrier(); //this person has ball
...
void engage(){
 ...
 for(int blocker = 0; blocker < blockers.size(); ++blocker){
  ...
  if(estimateBlock.size() > 0){//Shoot
    ...
    else if(get_dist(*getCarrier(), getBadGoalie()) <= engage_distance / 4){//if right up in the goalie's face, but too chicken to do anything...
      if(!(rand()%4)){shoot(estimateBlock);}//1:4 why not just go for it?
      else if(rand()%2>0){vector <Player*> emptyvec; pass(emptyvec);}//1:2 pass to just
    }//end else if right next to goal with no purpose in life
    else if(estimateBlock.size() == 0){recap << endl;}//just keep swimming
    return;    
  }//end else dribble
}//end engage()
